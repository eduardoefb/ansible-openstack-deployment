...
export PODMAN_IMAGE_VERSION="0.0.1"
export PODMAN_NAME="buildbm"
export AUTOINSTALL_CONFIG_DIR="config/autoinstall"
export OPENSTACK_CONFIG_FILE="config/openstack/config_2204.yml"
...
for i in ${SRV_LIST[*]}; do
    ...
    podman exec -it ${PODMAN_NAME} mv /root/ubuntu-autoinstall.iso /var/www/html/${i}.iso     
done
...
podman stop ${PODMAN_NAME} >/dev/null 2>&1
cp ${OPENSTACK_CONFIG_FILE} ../ansible/config.yml
cd ../ansible
